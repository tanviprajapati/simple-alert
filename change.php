...
<div id="radio">
    <input type="radio" id="radio1" name="radio" value="true" checked="checked" /><label for="radio1">Yes</label>
    <input type="radio" id="radio2"  name="radio" value="false" /><label for="radio2">No</label>
</div>
		</td>
        </tr>
        <tr valign="top">
...
function myPlugin_admin_scripts() {
	...
	if ( isset($_GET['page']) && $_GET['page'] == 'simple-alert/SimpleAlert.php' ) {
		wp_enqueue_script('jquery');
		wp_enqueue_script('jqueryw','https://code.jquery.com/ui/1.12.1/jquery-ui.js');
		//wp_enqueue_script('jquery-ui-button');
		wp_enqueue_script( 'alert-script',plugins_url( '/js/semantic.js', __FILE__ ) );
		wp_enqueue_script( 'alert-script3',plugins_url( '/js/myform.js', __FILE__ ) );
		
		wp_enqueue_style('adminstyle1', plugins_url( 'css/semantic.css' , __FILE__ ), false, '2.2', 'all' );
	...
	//wp_enqueue_script('myscriptjs',plugins_url( '/js/fancybox.js', __FILE__ ));
